...
class MakeSlide {
  ...
  next(callBack) {
    ...
    if (this.position !== 0 && this.position !== this.list.length - 1) {
      calc =
        (this.translateX + this.list[this.position + 1].position) * -1 +
    ...
    if (this.position === 0) {
      calc =
        -this.list[this.getPosition()].position +
        this.width / 2 -
        this.list[this.getPosition()].width / 2;
  ...
  dragableSlide(pt) {
    ...
    if (this.translateY <= this.getLimitY() && y < 0) {
      y = this.activeZoom ? -0.4 : 0;
    }
    if (this.translateY >= 0 && y > 0) {
      y = this.activeZoom ? 0.4 : 0;
    }
  ...
  getLimitX() {
    return (
      ...
      (this.width / this.scale) * this.scaleCalc
    );
  }
  ...
  getPosition() {
    const point = this.list.findIndex(
      (a, i) =>
        a.position >= this.width / 2 &&
  ...
  getPointX() {
    if (this.translateX <= -this.list[this.list.length - 2].position)
      return this.list.length - 1;
    const point = this.list.findIndex(
      (a, i) =>
        a.position >= this.translateX * -1 &&
